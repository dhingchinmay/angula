...
export class PropertyComponent implements OnInit {
  ...
  proc = [
    ...
    { "id": 9, "title": "Balcony View", "description": "2 BHK House for rent in Ratanpur 900 sqft", "price": "20000/Month", "propertyImage": "assets/img/44.jpg/", "details": "2 Bedroom, 2 Bathroom, Kitchen", booked: false },
  ];
  isBooked = false;
  ...
  ngOnInit() {
    ...
    propertyList.subscribe((data: any) => {
      for (let i in data) {
        this.proc.push({
          ...data[i],
          id: i
    ...
    for (let i in this.bookedProperties) {
      for (let j in this.proc) {
        if (this.bookedProperties[i] === this.proc[j].id) {
          this.proc[j].booked = true;
        }
      }
    }
    console.log(this.proc)
